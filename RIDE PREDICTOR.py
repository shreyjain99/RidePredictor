...
# ## Data Cleaning
# 
# Doing univariate analysis and removing outlier/illegitimate values which may be caused due to some error

# In[10]:


#Few datapoints along with all our features
...
# spotting only first 100 outliers on the map, plotting all the outliers will take more time
sample_locations = outlier_locations.head(10000)
for i,j in sample_locations.iterrows():
    if int(j['pickup_latitude']) != 0:
...
# <b style='font-size:18px'>The avg speed in Newyork speed is 12.45miles/hr, so a cab driver can travel <font color='blue'> 2 miles per 10min on avg.</font> </b>
...
def find_min_distance(cluster_centers, cluster_len):
    ...
    print ("On choosing a cluster size of ",cluster_len,"\nAvg. Number of Clusters within the vicinity (i.e. intercluster-distance < 2):", np.ceil(sum(less2)/len(less2)), "\nAvg. Number of Clusters outside the vicinity (i.e. intracluster-distance > 2):", np.ceil(sum(more2)/len(more2)),"\nMin inter-cluster distance = ",min_dist,"\n---")
...
# and makings sure that the minimum inter cluster should not be very less
...
# 1. INCORPORATE FOURIER TRANSFORM FEATURES AS FEATURES INTO REGRESSION MODELS
# 2. PERFORM HYPERPARAMETER TUNING FOR REGRESSION MODELS
# 3. TRY MORE REGRESSION MODELS AS WELL AS ARTIFICIAL NEURAL NETS (ANN)
